...
import { Calendar, Clock, ArrowRight, Sparkles, MessageCircle, Heart, User } from "lucide-react";
...
export const metadata = {
  title: "المدونة",
  description: "مشاركات وإسهامات أعضاء AI.DY. مقالات، مراجعات، تجارب، ونقاشات.",
...
export default async function BlogPage() {
  ...
  if (supabase) {
    const { data } = await supabase
      .from("user_posts")
      .select(
        "id, slug, title, excerpt, cover_url, tags, reading_time, published_at, likes_count, comments_count, author_id"
      )
      .eq("status", "published")
      .order("published_at", { ascending: false })
      .limit(50);
    posts = (data as UserPost[] | null) ?? [];
    ...
    if (posts.length > 0) {
      ...
      authors = Object.fromEntries(
        ((profiles as Author[] | null) ?? []).map((a) => [a.id, a])
      );
    }
  }
      ...
      <div className="flex flex-col flex-1">
        <section className="mx-auto w-full max-w-6xl px-6 py-20 text-center">
          ...
          <p className="mb-2 text-zinc-600 dark:text-zinc-400">
            شاركنا تجربتك مع أدوات الذكاء الاصطناعي
          </p>
          <p className="text-sm text-zinc-500">
            (المدونة لسه جديدة — أول بوستات هتظهر هنا قريباً)
          </p>
        </section>
      </div>
    );
  ...
  const featuredAuthor = authors[featured.author_id];
          ...
          <div className="mb-6 flex items-end justify-between">
            <div>
              <p className="mb-2 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.18em] text-violet-600 dark:text-violet-400">
                <Sparkles className="h-3 w-3" /> آخر المشاركات
              </p>
                ...
                <div className="mt-4 flex items-center gap-2 text-sm">
                  {featuredAuthor.avatar_url ? (
                    ...
                    <div className="flex h-6 w-6 items-center justify-center rounded-full bg-violet-100 text-xs font-semibold text-violet-700 dark:bg-violet-950/50 dark:text-violet-300">
                      {(featuredAuthor.display_name ?? "?").charAt(0)}
                    </div>
                  )}
                  <span className="font-medium text-zinc-700 dark:text-zinc-300">
                    {featuredAuthor.display_name ?? "عضو"}
                  </span>
                </div>
              )}
                ...
                <div className="flex flex-1 flex-col p-5">
                  {author && (
                    <div className="mb-2 flex items-center gap-1.5 text-xs text-zinc-500">
                      {author.avatar_url ? (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img src={author.avatar_url} alt="" className="h-4 w-4 rounded-full" />
                      ) : (
                      ...
                      <span>{author.display_name ?? "عضو"}</span>
                    </div>
                  )}
                  {p.tags.length > 0 && (
