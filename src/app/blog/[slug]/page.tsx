...
import { Calendar, Clock, ArrowRight, Twitter, Linkedin, Link2, ChevronRight } from "lucide-react";
...
export default async function BlogPostPage({
  ...
  let relatedTools: Array<{ id: string; slug: string; name: string; tagline: string | null; pricing_type: string | null }> = [];
  if (targetToolSlugs.length > 0) {
    const { data } = await supabase
      .from("tools")
      .select("id, slug, name, tagline, pricing_type")
      .in("slug", targetToolSlugs)
      .eq("is_published", true)
      .limit(6);
    relatedTools = (data as typeof relatedTools) ?? [];
  }

  // Related posts (exclude current)
  const { data: related } = await supabase
    .from("articles")
    .select("id, slug, title, excerpt, cover_url, reading_time, published_at")
    .eq("status", "published")
    .neq("id", post.id)
  ...
  const relatedPosts = (related as Array<{
    ...
    cover_url: string | null; reading_time: number | null; published_at: string | null;
  }> | null) ?? [];
        ...
        <div className="mx-auto max-w-3xl px-6 py-10 sm:py-14">
          ...
          <div className="mt-6 flex flex-wrap items-center gap-4 border-t border-zinc-200 pt-6 text-sm text-zinc-500 dark:border-zinc-800 dark:text-zinc-400">
            ...
            <div className="ms-auto flex items-center gap-2">
              ...
              <a
                href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://ai-dy-six.vercel.app/blog/${post.slug}`)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="rounded-full border border-zinc-200 p-2 text-zinc-500 transition hover:border-zinc-300 hover:bg-zinc-50 hover:text-zinc-900 dark:border-zinc-800 dark:hover:border-zinc-700 dark:hover:bg-zinc-800 dark:hover:text-zinc-100"
                ...
                onClick={() => {
                  if (typeof navigator !== "undefined" && navigator.clipboard) {
                    void navigator.clipboard.writeText(
                      `https://ai-dy-six.vercel.app/blog/${post.slug}`
                    );
                  }
                }}
              >
              ...
              >
                ...
                <div className="flex flex-1 flex-col p-4">
                  ...
                  <div className="mt-auto pt-2 text-xs text-zinc-500">
                    {r.reading_time && `${r.reading_time} د قراءة`}
                  </div>
                </div>
              </Link>
            ))}
