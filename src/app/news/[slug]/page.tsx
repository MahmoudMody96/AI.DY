...
export async function generateMetadata({
  ...
  if (!supabase) {
    return { title: "خبر", alternates: { canonical: `/news/${slug}` } };
  }
  ...
  if (!post) {
    return { title: "خبر غير موجود", alternates: { canonical: `/news/${slug}` } };
  }
...
export default async function NewsPostPage({
  ...
  const shareUrl = `${getSiteUrl()}/news/${post.slug}`;
      ...
      <header className="border-b border-zinc-200 dark:border-zinc-800">
        <div className="mx-auto max-w-3xl px-6 py-10 sm:py-14">
          <nav className="mb-6 flex items-center gap-1.5 text-xs text-zinc-500 dark:text-zinc-400">
            ...
            <Link href="/news" className="hover:text-violet-600">الأخبار</Link>
            <ChevronRight className="h-3 w-3 rotate-180" />
          ...
          <div className="mb-3 inline-flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-violet-600 dark:text-violet-400">
            <Newspaper className="h-3.5 w-3.5" /> خبر
          </div>
